此DataTables的jQuery插件（v1.9.x）开发出先进的访问功能允许高度可配置的访问HTML表格的愿望。

有关详细的安装，使用和API说明，请参阅数据表的Web页面：http://www.datatables.net

问题，功能要求和错误报告（等）都可以问的DataTable论坛：http://www.datatables.net/forums/

可以发现在 media/js/ 目录这个存档的DataTable源。

数据表是采用双重授权发布，使用的GPL V2（许可证gpl2.txt）和BSD风格的许可证（许可证bsd.txt）的。
你可以选择你想使用的DataTable下的两个许可证。
这些许可证的详细信息，请参阅相应的许可证文件。
你可以自由地使用，修改和分发本软件，但必须保持所有的版权信息。
